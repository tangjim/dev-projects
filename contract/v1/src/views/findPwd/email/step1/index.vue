<template>
    <div class="step-main">
        <div class="form-box">
            <my-input v-model="formData.email" ref="email" :maxlength="11" :placeholder="'请输入邮箱地址'" :rules="rules.email" />
        </div>
        <van-button class="form-btn" @click="sendSms" v-waves>{{btnTxt}}</van-button>

        <div class="step-btn">
            <router-link class="email-register" to="/find/pwd/m">手机号码找回</router-link>
        </div>
    </div>
</template>
