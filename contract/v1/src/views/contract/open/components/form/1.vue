    ...
    <div class="components-main">
        <div class="form">
           <div class="select-plane">
               <div class="input-group w214"><my-select :options="typeList" :value="order.type" v-on:selected="selectedType"></my-select></div>
               <div class="input-group ml10 w176">
                   <div class="form-select" @click="triggerSelectLeverPopup">
                       <div class="text">{{getText(order.lever)}}</div>
                       <svg class="icon" aria-hidden="true" :class="{'open':selectLeverOpen}"><use :xlink:href="'#icon-jiantou1-copy'"></use></svg>
                   </div>
               </div>
           </div>
            ...
            <div class="price_info">
                <div><span>成本</span><span>7.8812&nbsp;USDT</span></div>
                <div><span>可用</span><span>12.8812&nbsp;USDT</span></div>
                <div><span>可开张数</span><span>2张</span></div>
            </div>
        ...
        <van-popup v-model="selectLeverOpen" :close-on-popstate="true" :style="{ position:'absolute', top:'7.9rem', width:'9.35rem'}" get-container=".main">
            <div class="lever-plane">
                ...
                <div class="box">
                    ...
                    <div class="handle">
                        <button @click="confirmLeverSelected" v-waves>确定</button>
                    </div>
                </div>
            </div>
        </van-popup>
...
<script>
    ...
    export default {
        name: "contract-open-components-1",
        components: {MySelect},
        computed: {
            ...mapState({
...
<style scoped>
    ...
    .form .select-plane {display: inline-block}
    .form .select-plane .input-group {float: left;}
    ...
    .form .price_info {width: 100%;margin-top: 20px;}
    .form .price_info > div {width: 100%;overflow: hidden;}
    .form .price_info span {font-size:24px;font-family:PingFang SC;font-weight:400;line-height:34px;color:rgba(140,161,169,1);}
    .form .price_info span:nth-child(1) {float: left;}
    .form .price_info span:nth-child(2) {float: right;}

    .form .handle {width: 100%;margin-top: 20px}
    .form .handle button {width: 100%;height:64px;border-radius:6px;font-size:24px;font-family:PingFang SC;font-weight:400;line-height:64px;color:rgba(255,255,255,1);text-align: center;margin-bottom: 12px;}
    .form .handle button.green {background:rgba(76,185,113,1);}
    .form .handle button.red {background:rgba(237,101,94,1);}
    .form .handle button:last-child {margin-bottom: 0}
