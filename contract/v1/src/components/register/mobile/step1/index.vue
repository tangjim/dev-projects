<template>
    <div class="step-main">
        <h3 class="title">手机号码注册</h3>
        <div class="form-box">
            <!--<div class="form-group">-->
                <!--<input type="text" ref="mobilePhone" v-model="formData.mobilePhone" maxlength="11" placeholder="请输入您常用的手机号码"/>-->
            <!--</div>-->
            <my-input v-model="formData.mobilePhone" ref="mobilePhone" :maxlength="11" :placeholder="'请输入您常用的手机号码'" :rules="rules.mobilePhone" />
        </div>
        <van-button class="form-btn" :loading="sms.sendState===1" round @click="sendSms">{{btnTxt}}</van-button>

        <div class="step-btn">
            <router-link class="email-register" to="/authorize/register/e">邮箱注册</router-link>
    ...
    export default {
        ...
        data() {
            return {
                ...
                rules: {
                    mobilePhone: [{required:true,message:"请输入您常用的手机号码"}],
                },
                subState: 0,
                btnTxt: "下一步",
                // showPassword: false,
        ...
        methods: {
            ...
            sendSms: function() {
                ...
                // 开启短信倒计时
                // send({mobilePhone: this.formData.mobilePhone, type:1,applyType:1}, "reg").then(res => {
                //     // console.log(res);
                //     Toast(res.message);
                //     if (res.code === 1000) {
                //         if (res.data.code) {
